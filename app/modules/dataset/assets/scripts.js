...
// ======================================================
// OPCIONAL: borrar ficheros GPX del servidor al pulsar
// un botón que tenga class="delete_gpx" y data-filename
// ======================================================
document.addEventListener("click", function (event) {
    if (event.target && event.target.classList.contains("delete_gpx")) {
        ...
        // Si ese botón está dentro del bloque .gpx_form, lo quitamos del DOM
        const gpxForm = event.target.closest(".gpx_form");
        if (gpxForm) gpxForm.remove();
    }
});

window.onload = function () {

    // existe en tu proyecto original
    ...
    document.getElementById('upload_button').addEventListener('click', function () {
        ...
        if (check) {
            ...
            // 1) Recolectar formularios base y UVL (como ya hacías)
            ...
            // 2) *** NUEVO *** Recolectar también los inputs de GPX
            //    (dentro del contenedor #uploaded_gpx_form y con clase .gpx_form)
            document
                .querySelectorAll('#uploaded_gpx_form .gpx_form input, #uploaded_gpx_form .gpx_form select, #uploaded_gpx_form .gpx_form textarea')
                .forEach((input) => {
                    if (input.name) {
                        formData[input.name] = formData[input.name] || [];
                        formData[input.name].push(input.value);
            ...
            // OJO: este patrón ya existía. Mantengo el comportamiento tal cual.
            // (WTForms aceptará los valores como listas serializadas en el backend actual)
            ...
            if (checked_orcid && checked_name) {
                fetch('/dataset/upload', {
                    ...
                    .catch(error => {
                        ...
                        hide_loading();
                        write_upload_error("Unexpected error while uploading.");
                    });
            } else {
